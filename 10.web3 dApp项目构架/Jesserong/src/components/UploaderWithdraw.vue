    ...
    methods: {
      ...
      async withdraw() {
        ...
        try {
          ...
          const file = await contract.files(this.fileId);
          const uploaderAddress = file.uploader.toString().toLowerCase();
          console.log(uploaderAddress);
          console.log(this.address[0]);
          if(this.address[0].toLowerCase() !== uploaderAddress) {
            alert("You are not the uploader.");
            return;
          }

          let tx = await contract.uploaderWithdraw(amount,this.fileId,options);
          await tx.wait();
        } catch(err) {
            console.log(err);
